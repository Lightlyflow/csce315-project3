...
def getUserByEmail(email: str):
    ...
    :return: List containing user ID and username
    """
...
def addUser(email: str):
    """
    Creates user in database with given email. Username is by default what's in front of @ symbol.
    :param email: user email
    :return: None
    """
...
def getEmployeeByEmail(email: str):
    ...
    :param email: employee email (not the same as user email!)
    :return: List containing name, employee ID, manager status, preferred name, and admin status
    """
...
def emailExists(email: str):
    """
    Checks if user email exists in either users or employees.
    :param email: user email
    :return: [[0]] if not found, otherwise [[nonzero value]]
    """
    return execute(
        f"SELECT count(*) FROM user_table WHERE EXISTS ( SELECT 1 FROM employee_table WHERE email='{email}' ) OR EXISTS ( SELECT 1 FROM user_table WHERE email='{email}' );")
